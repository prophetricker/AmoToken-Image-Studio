...
2026-07-09 部署 `0d1ed0e` 后验证：

- 当前镜像：`amotoken/nova-image-studio:v0.10-0d1ed0e`
- 服务器源码：`0d1ed0e fix: sanitize gif encoding failures`
- Compose 镜像行：`amotoken/nova-image-studio:v0.10-0d1ed0e`
- 本机首页：`200`
- 容器内访问爱词元主服务状态：`200`
- 队列：空闲，`processingCount=0`、`queuedCount=0`、`remainingQueueSlots=40`
- 端口：`127.0.0.1:3001->3000/tcp`
- 公网 `3001`：TCP 连接失败，符合预期
- `https://img.amotoken.cc/`：未带 Basic Auth 返回 `401`
- Nova 内存：约 `18.77MiB / 1.918GiB`
- NewAPI 容器内存：约 `75.98MiB / 1.918GiB`
- 根分区：约 `39G` 总量，`7.8G` 可用，使用率约 `79%`
- Nova 数据目录：约 `20M`，共 `49` 个文件
- 提示词图片缓存：`46` 个文件，约 `16M`，未见无上限膨胀
- 任务数据库 WAL：约 `4.1M`
...
2026-07-09 部署 `69aec87` 后验证：

- 当前镜像：`amotoken/nova-image-studio:v0.10-69aec87`
- 服务器源码：`69aec87 feat: explain 4k gray-test usage`
- Compose 镜像行：`amotoken/nova-image-studio:v0.10-69aec87`
- 本机首页：`200`
- 容器内访问爱词元主服务状态：`200`
- 队列：空闲，`processingCount=0`、`queuedCount=0`、`remainingQueueSlots=40`
- 端口：`127.0.0.1:3001->3000/tcp`
- 公网 `3001`：TCP 连接失败，符合预期
- `https://img.amotoken.cc/`：未带 Basic Auth 返回 `401`
- Nova 内存：约 `20MiB / 1.918GiB`
- NewAPI 容器内存：约 `56.46MiB / 1.918GiB`
- 根分区：约 `39G` 总量，`6.5G` 可用，使用率约 `83%`
- Nova 数据目录：约 `20M`，共 `52` 个文件
- 提示词图片缓存：`49` 个文件，约 `16M`，未见无上限膨胀
- 任务数据库 WAL：约 `4.1M`
...
2026-07-09 部署 `c7700da` 后验证：

- 当前镜像：`amotoken/nova-image-studio:v0.10-c7700da`
- 服务器源码：`c7700da fix: sanitize canvas task creation failures`
- Compose 镜像行：`amotoken/nova-image-studio:v0.10-c7700da`
- 本机首页：`200`
- 容器内访问爱词元主服务状态：`200`
- 队列：空闲，`processingCount=0`、`queuedCount=0`、`remainingQueueSlots=40`
- 端口：`127.0.0.1:3001->3000/tcp`
- 公网 `3001`：TCP 连接失败，符合预期
- `https://img.amotoken.cc/`：未带 Basic Auth 返回 `401`
- Nova 内存：约 `18.96MiB / 1.918GiB`
- NewAPI 容器内存：约 `67.14MiB / 1.918GiB`
- 根分区：约 `39G` 总量，`7.8G` 可用，使用率约 `79%`
- Nova 数据目录：约 `20M`，共 `52` 个文件
- 提示词图片缓存：`49` 个文件，约 `16M`，未见无上限膨胀
- 任务数据库 WAL：约 `4.1M`；历史任务状态含 `failed|1`，队列无堆积
- Docker build cache：约 `14.61GB` 可回收，仍只记录告警，不自动清理
- 日志：启动正常，未见失败堆积或缓存错误

2026-07-09 追加 `c7700da` 部署后短时运行观察：

- 当前镜像仍为 `amotoken/nova-image-studio:v0.10-c7700da`
- 运行约 `3` 分钟后再次采集只读快照，Nova 端口仍为 `127.0.0.1:3001->3000/tcp`
- Nova 内存：约 `19.2MiB / 1.918GiB`
- NewAPI 容器内存：约 `58.77MiB / 1.918GiB`
- 根分区：约 `39G` 总量，`7.8G` 可用，使用率约 `79%`
- Nova 数据目录：约 `20M`，共 `52` 个文件
- 提示词图片缓存：`49` 个文件，约 `16M`，与部署后快照一致，未见增长异常
- 任务数据库 WAL：约 `4.1M`；历史任务状态仍含 `failed|1`，队列无堆积
- 队列接口：`processingCount=0`、`queuedCount=0`、`remainingQueueSlots=40`
- Docker build cache：约 `14.61GB` 可回收，仍只记录告警，不自动清理
- 日志：仅见启动、图片目录、提示词缓存目录和配置日志，未见失败堆积或缓存错误
